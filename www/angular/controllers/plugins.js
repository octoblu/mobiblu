  ...
  .controller('PluginCtrl', function($rootScope, $scope, $routeParams, $timeout, $location, Utils, Subdevices, Plugins, PluginInstances) {
    ...
    $scope.findOne = function(cb) {
      ...
      Plugins.ready().then(function() {
        ...
        var plugins = _.where($scope.plugins, {
          name: $routeParams.pluginName
        });
    ...
    $scope.deleteDevice = function() {
      ...
      _.remove($scope.plugin.subdevices, {
        uuid: $scope.subdevice.uuid
      });
    ...
    $scope.loadSubdevice = function() {
      ...
      if (uuid && uuid.length) {
        ...
        $scope.findOne(function() {
          ...
          var subdevice = _.where($scope.plugin.subdevices, {
            uuid: uuid
          });
    ...
    $scope.writePlugin = function(init) {
      ...
      if ($scope.subdevice) {
        var x = _.findIndex($scope.plugin.subdevices, {
          uuid: $scope.subdevice.uuid
        });
        if (~x) {
