...
angular.module('main.plugins')
  .service('Plugins', function($window, $rootScope, $http, FileSystem, SkynetConn, PluginInstances, Utils, Skynet, Subdevices, $q) {
    ...
    service.initPlugin = function(plugin) {
      ...
      if (plugin.enabled) {
        var promises = _.map(plugin.subdevices, function(subdevice){
          return service.initDevice(subdevice);
        });
    ...
    service.initDevice = function(subdevice) {
      var deferred = $q.defer();
      PluginInstances.initDevice(subdevice, function(p) {
      ...
      if (!~found || !service.pluginIsLoaded(name)) {
        ...
        registerPlugin(name)
          .then(function() {
            ...
            loadScript(plugin)
              .then(function() {
                return service.initPlugin(plugin);
              })
